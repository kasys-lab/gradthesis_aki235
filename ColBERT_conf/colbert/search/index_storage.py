...
class IndexScorer(IndexLoader, CandidateGeneration):
    ...
    def rank(self, config, Q, confidence_layer, confidence_search=False, confidence_params=[], filter_fn=None, pids=None):
        with torch.inference_mode():
            ...
            scores, pids = self.score_pids(config, Q, confidence_layer, pids, centroid_scores, confidence_search=confidence_search, confidence_params=confidence_params)
    ...
    def score_pids(self, config, Q, confidence_layer, pids, centroid_scores, confidence_search=False, confidence_params=[]):
        """
        ...
        """# _search_all_Qでクエリ一つ一つ処理してるから1しかありえないのでは？→他のところからscore_pidsを呼び出したりするかもしれない
        ...
        if Q.size(0) == 1:
            return colbert_score_packed(Q, confidence_layer, D_packed, D_mask, config, confidence_search=confidence_search, confidence_params=confidence_params), pids
