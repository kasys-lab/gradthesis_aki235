...
class Searcher:
    ...
    def encode(self, text: TextQueries, full_length_search=False):
        ...
        Q, confidence_layer = self.checkpoint.queryFromText(queries, bsize=bsize, to_cpu=True, full_length_search=full_length_search)

        return Q, confidence_layer
    ...
    def search_all(self, queries: TextQueries, k=10, confidence_search=False, confidence_params=[], filter_fn=None, full_length_search=False, qid_to_pids=None):
        queries = Queries.cast(queries)
        queries_ = list(queries.values())

        Q, confidence_layer = self.encode(queries_, full_length_search=full_length_search)

        return self._search_all_Q(queries, Q, k, confidence_layer, confidence_search=confidence_search, confidence_params=confidence_params, filter_fn=filter_fn, qid_to_pids=qid_to_pids)

    def _search_all_Q(self, queries, Q, k, confidence_layer, confidence_search=False, confidence_params=[], filter_fn=None, qid_to_pids=None):
            ...
            list(
                zip(
                    *self.dense_search(
                        ...
                        confidence_layer[query_idx],
                        k,
                        confidence_search=confidence_search,
    ...
    def dense_search(self, Q: torch.Tensor, confidence_layer, k=10, confidence_search=False, confidence_params=[], filter_fn=None, pids=None):
        if k <= 10:
        ...
        pids, scores = self.ranker.rank(self.config, Q, confidence_layer, confidence_search=confidence_search, confidence_params=confidence_params, filter_fn=filter_fn, pids=pids)
